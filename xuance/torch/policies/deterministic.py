...
class GNNBasicQnetwork(Module):
    ...
    def __init__(self,
                 ...
                 device: Optional[Union[str, int, torch.device]] = None,
                 use_distributed_training: bool = False):
        super(GNNBasicQnetwork, self).__init__()
        self.action_dim = action_space.n
        self.representation = representation
        self.target_representation = deepcopy(representation)
        self.representation_info_shape = self.representation.output_shapes
        self.eval_Qhead = BasicQhead(self.representation.output_shapes['state'][0]+11, self.action_dim, hidden_size,
                                     normalize, initialize, activation, device)
        self.target_Qhead = deepcopy(self.eval_Qhead)
    ...
    def forward(self, observation: Union[np.ndarray, dict]):
        ...
        Returns:
            ...
            evalQ: The evaluated Q-values.
        """
        all_env_graph_data, all_else_feature = self.build_gnn_observation(observation)
        gnn_outs = self.representation(all_env_graph_data)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        torch_else_feature = torch.as_tensor(all_else_feature, dtype=torch.float32, device=device)
        Qhead_cat_data = torch.cat((gnn_outs['state'], torch_else_feature), dim=-1)
        evalQ = self.eval_Qhead(Qhead_cat_data)
        argmax_action = evalQ.argmax(dim=-1)
        return gnn_outs, argmax_action, evalQ
    
    def build_gnn_observation(self, observation: Union[np.ndarray, dict]):
        # 提取图数据,
        all_env_graph_data = []
        all_else_feature = []
        for bs in range(len(observation)):
            data = observation[bs]
            graph_data = data[:65]
            else_feature = data[65:]
            # 将 graph_data 按顺序分为5个向量然后组成数组
            graph_data = graph_data.reshape(5, 13) 
            all_env_graph_data.append(graph_data)
            all_else_feature.append(else_feature)
        # if obs 是张量
        if isinstance(observation, Tensor):
            all_env_graph_data = torch.stack(all_env_graph_data, dim=0)
            all_else_feature = torch.stack(all_else_feature, dim=0)
            return all_env_graph_data, all_else_feature
        return np.array(all_env_graph_data), np.array(all_else_feature)
    ...
    def target(self, observation: Union[np.ndarray, dict]):
        ...
        """
        all_env_graph_data, all_else_feature = self.build_gnn_observation(observation)
        gnn_outs = self.target_representation(all_env_graph_data)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        torch_else_feature = torch.as_tensor(all_else_feature, dtype=torch.float32, device=device)
        Qhead_cat_data = torch.cat((gnn_outs['state'], torch_else_feature), dim=-1)
        
        targetQ = self.target_Qhead(Qhead_cat_data)
        argmax_action = targetQ.argmax(dim=-1)
        return gnn_outs, argmax_action.detach(), targetQ.detach()
