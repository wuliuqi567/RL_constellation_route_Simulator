...
class GNN_MAAC_Policy(Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(GNN_MAAC_Policy, self).__init__()
        self.device = device
        ...
        for key in self.model_keys:
            ...
            dim_actor_in, dim_actor_out, dim_critic_in, dim_critic_out = self._get_actor_critic_input(
                self.n_actions[key],
                self.actor_representation[key].output_shapes['state'][0] + 10,
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Optional[Tensor] = None,
        ...
        for key in agent_list:
            if observation.get("graph_data"):
                env_node_features = []
                graph_data = observation["graph_data"]
                for env_data_list in graph_data:
                    node_features = []
                    for nx_graph_data in env_data_list:
                        center_node_feature = self.actor_representation[key](nx_graph_data)
                        node_features.append(center_node_feature)
                    final_node_features = torch.stack(node_features, dim=0)
                    env_node_features.append(final_node_features)
                
                env_node_features_torch = torch.cat(env_node_features, dim=0)
                # rnn_hidden_new[key] = (outputs['rnn_hidden'], outputs['rnn_cell'])
                if self.use_parameter_sharing:
                    obs_tensor = torch.from_numpy(observation["agent_0"]).to(self.device)
                    actor_input = torch.concat([obs_tensor, env_node_features_torch, agent_ids], dim=1)
                else:
        ...
        return rnn_hidden_new, pi_dists, env_node_features
    ...
    def get_values(self, observation: Dict[str, Tensor], agent_ids: Tensor = None, agent_key: str = None,
        ...
        for key in agent_list:
            ...
            # graph_data 的形状为 (batch_size, num_nodes, node_features)
            # 将其变为 (batch_size * num_nodes, num_nodes*node_features)
            if graph_data is not None:
                mulit_graph_data = []
                for one_graph_data in graph_data:
                    shape = one_graph_data.shape
                    one_env_graph_data = one_graph_data.view(1, -1).repeat(shape[0], 1)
                    mulit_graph_data.append(one_env_graph_data)
                graph_data = torch.cat(mulit_graph_data, dim=0)
            if self.use_rnn:
                outputs = self.critic_representation[key](observation[key], *rnn_hidden[key])
                rnn_hidden_new[key] = (outputs['rnn_hidden'], outputs['rnn_cell'])
            else:
                if graph_data is not None:
                    obs_inputs = torch.from_numpy(observation[key]).to(self.device)
                    rep_inputs = torch.concat([obs_inputs, graph_data], dim=-1)
                outputs = self.critic_representation[key](rep_inputs)
                rnn_hidden_new[key] = [None, None]

            if self.use_parameter_sharing:
                if graph_data is not None:
                    critic_input = torch.concat([outputs['state'], agent_ids, graph_data], dim=-1)
...
class MAAC_Policy(Module):
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Optional[Tensor] = None,
        ...
        for key in agent_list:
            if hasattr(observation, "graph_data"):
                graph_data = observation["graph_data"]
                outputs = self.actor_representation[key](graph_data, *rnn_hidden[key])
                rnn_hidden_new[key] = (outputs['rnn_hidden'], outputs['rnn_cell'])
            else:
                if self.use_rnn:
                    outputs = self.actor_representation[key](observation[key], *rnn_hidden[key])
