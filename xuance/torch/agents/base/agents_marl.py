...
class MARLAgents(ABC):
    ...
    def __init__(self,
        ...
        if config.logger == "tensorboard":
            ...
            self.use_wandb = True
            self.use_swanlab = False
        elif config.logger == "swanlab":
            config_dict = vars(config)
            log_dir = config.log_dir
            log_dir = Path(os.path.join(os.getcwd(), config.log_dir))
            if self.rank == 0:
                create_directory(str(log_dir))
            else:
                while not os.path.exists(str(log_dir)):
                    pass
            swanlab.init(config=config_dict,
                        project=config.project_name,
                        entity=config.wandb_user_name,
                        notes=socket.gethostname(),
                        dir=log_dir,
    ...
    def _build_gnn_representation(self, representation_key: str, config: Namespace) -> Module:
        """
        Build representation for policies.
        ...
        for key in self.model_keys:
            if self.use_rnn:
                ...
                hidden_sizes = config.representation_hidden_size if hasattr(config,
                                                                            "representation_hidden_size") else None
            input_representations = dict(
                node_dim=config.gnn_node_dim if hasattr(config, "gnn_node_dim") else None,
                edge_dim=config.gnn_edge_dim if hasattr(config, "gnn_edge_dim") else None,
                hidden_dim=config.gnn_hidden_dims if hasattr(config, "gnn_hidden_dims") else None,
                layer_node_dims=config.gnn_layer_node_dims if hasattr(config, "gnn_layer_node_dims") else None,
                hidden_sizes=hidden_sizes,
                normalize=NormalizeFunctions[config.normalize] if hasattr(config, "normalize") else None,
                initialize=nn.init.orthogonal_,
                activation=ActivationFunctions[config.activation],
    ...
    def  _build_inputs_with_gnn_rep(self,
                      obs_dict: List[dict],
                      avail_actions_dict: Optional[List[dict]] = None):
        """
        Build inputs for representations before calculating actions.
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                if self.use_actions_mask:
                    avail_actions_input = {k: np.array([data[k] for data in avail_actions_dict]).reshape([bs, -1])
                                           for k in self.agent_keys}
        # if obs_dict has graph_data attr
        # if hasattr(obs_dict[0], 'graph_data'):
        graph_data = [data['graph_data'] for data in obs_dict]
        obs_input['graph_data'] = graph_data
    ...
    def finish(self):
        if self.use_wandb or self.use_swanlab:
            wandb.finish()
        else:
            self.writer.close()
        if self.distributed_training:
