...
class GNN_PLUS_MAPPO_Agents(IPPO_Agents):
    ...
    def __init__(self,
        ...
        super(GNN_PLUS_MAPPO_Agents, self).__init__(config, envs)
    ...
    def _build_policy(self) -> Module:
        ...
        A_representation = self._build_gnn_representation(self.config.representation, self.config)
        if self.use_global_state:
            dim_obs_all = sum(self.state_space.shape)
        else:
            dim_obs_all = sum([sum(self.observation_space[k].shape) for k in self.agent_keys])
        gnndim = A_representation["agent_0"].output_shapes['state'][0]
        space_critic_in = {k: (dim_obs_all + gnndim * len(self.agent_keys), ) for k in self.agent_keys}
        
        C_representation = self._build_representation("Basic_MLP", space_critic_in, self.config)
        # build policies
        ...
        elif self.config.policy == "GNN_Categorical_MAAC_Policy":
            policy = REGISTRY_Policy["GNN_Categorical_MAAC_Policy"](
                action_space=self.action_space, n_agents=self.n_agents,
                representation_actor=A_representation, representation_critic=C_representation,
                actor_hidden_size=self.config.actor_hidden_size, critic_hidden_size=self.config.critic_hidden_size,
                normalize=normalize_fn, initialize=initializer, activation=activation,
    ...
    def _build_critic_inputs(self, batch_size: int, obs_batch: dict,
                             state: Optional[np.ndarray], graph_node_features) -> dict:
        """
        ...
        if self.use_global_state:
            ...
            critic_input["graph_data"] = graph_node_features
        return critic_input
    ...
    def action(self,
        ...
        obs_input, agents_id, avail_actions_input = self._build_inputs_with_gnn_rep(obs_dict, avail_actions_dict)
        rnn_hidden_actor_new, pi_dists, env_node_features = self.policy(observation=obs_input,
                                                     agent_ids=agents_id,
                                                     avail_actions=avail_actions_input,
                                                     rnn_hidden=rnn_hidden_actor)
        if not test_mode:
            critic_input = self._build_critic_inputs(batch_size=n_env, obs_batch=obs_input, state=state, graph_node_features=env_node_features)
        ...
        return {"rnn_hidden_actor": rnn_hidden_actor_new, "rnn_hidden_critic": rnn_hidden_critic_new,
                "actions": actions_dict, "log_pi": log_pi_a_dict, "values": values_dict, "graph_rep": env_node_features}

    def get_value_next(self,
                       obs_dict: dict,
                       state: Optional[np.ndarray] = None,
                       rnn_hidden_critic: Optional[dict] = None,
                       avail_actions_dict: Optional[List[dict]] = None,
                        ...
                        **kwargs):
        """
        Returns critic values of the next step.
        Parameters:
            obs_dict (dict): Observations for each agent in self.agent_keys.
            state (Optional[np.ndarray]): The global state.
            rnn_hidden_critic (Optional[dict]): The RNN hidden states of critic representation.
    ...
    def values_next(self,
        ...
        if self.use_parameter_sharing:
            ...
            rnn_hidden_critic_new, values_out = self.policy.get_values(observation={key: critic_input, "graph_data": obs_dict["graph_data"]},
                                                                       agent_ids=agents_id,
