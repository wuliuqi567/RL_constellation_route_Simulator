...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, env_seed):
        ...
        if isinstance(self.observation_space, Dict):
            ...
            if self.obs_shape_low is not None:
                self.buf_obs_low = np.zeros(combined_shape(self.num_envs, self.obs_shape_low), dtype=np.float32)
...
class DummyVecEnv_MultiSatDomains(DummyVecEnv):
    def __init__(self, env_fns, env_seed):
        super(DummyVecEnv_MultiSatDomains, self).__init__(env_fns, env_seed)
        self.buf_obs = np.zeros(combined_shape(self.num_envs, self.obs_shape), dtype=np.float32)